# Meri — Complete User Guide

**Meri** is Capiro's AI chief of staff for government-affairs professionals. It answers questions
...
- **Open Meri:** click the Meri bubble (the floating button, bottom-right of any page). If you have
  pending proactive alerts, a red badge shows the count.
- **Side drawer:** Meri opens as a right-side drawer. **Drag its left edge** to resize (360px up to
  ~90% of the screen).
...
- **Access:** `standard_user` and above (client-portal users don't have Meri).
...
- **Title & client:** rename the conversation and assign it to a client with the title field +
  dropdown, then **Save**. Assigning a client scopes Meri's context to that client.
- **Archive / restore:** archive finished conversations (and restore them later).

## 3. What you can ask — capabilities
Meri chooses which data tools to call based on your question, then grounds its answer in the results.
...
## 4. How Meri shows its work (trust layer)
- **Thought-process timeline** (above the answer): Meri's plan for the turn + a live list of the
...
- **Tier badge:** a "deep" indicator when Meri uses its richer retrieval tier (vs. the fast tier for
  quick lookups) — chosen automatically by question type.
- **Collapsed summary:** when done, it folds to "Used N tools · M sources."

## 5. Citations
When Meri uses a source it cites it inline as **`[1]`, `[2]`**. Click a chip to open the **source
...
For **briefings/memos**, Meri runs a second pass that checks each factual claim against the retrieved
sources:
...
## 7. Conflict warnings
If Meri sees contradictory signals (e.g., what you said vs. what the data shows, or internal vs.
...
## 10. Memory — Meri remembers across conversations
- **Automatic:** Meri extracts durable facts from substantial exchanges and shows them as
  **"Meri learned: …"** chips, each with an **undo** to forget it.
- **Explicit:** say **"Remember that I go by Ninja"** / **"Remember our cadence is monthly"** — Meri
...
- **Edit** — on your last message; revise the wording and resend (Meri discards the old answer and
  everything after it).

## 12. Modes
- **Write mode** (toggle): Meri applies its output to the page you're on — e.g., fills/updates an
...
- **Research mode** (toggle) — **Deep Research**, a multi-step investigation:
  1. **Plan** — Meri proposes a research plan and asks a few **clarifying questions**;
...
With a connected M365 account (Settings → Integrations), Meri can **list** recent threads (optionally
by client), **reply** to a thread, and **send** email on your behalf. Emails can be associated with a
client for context.

## 14. Notes
Meri can **save notes** for you — and, tied to a meeting, an **encrypted, confidential meeting note**
with an access level (kept private/at-rest-encrypted).

## 15. Proactive alerts
Meri surfaces **alerts** without being asked — e.g., an upcoming meeting that has no prep notes, or a
client with no recent activity. They appear as a **badge** on the Meri button; open Meri to read and
**dismiss** them.
...
- **Tenant-isolated** — Meri only ever sees your firm's data.
...
| Make Meri remember / forget | "Remember that …" / **undo** the chip |
...
| Handle a proactive nudge | Open Meri → read the alert → dismiss |
