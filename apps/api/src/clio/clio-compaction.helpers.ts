...
export function buildCompactionPrompt(input: CompactionPromptInput): CompactionPrompt {
  const system = [
    'You maintain the rolling summary of a long conversation between a federal-lobbying professional and Clio, their AI assistant.',
  ...
  parts.push(
    ...
    ...input.turns.map((t) => `${t.role === 'assistant' ? 'Clio' : 'User'}: ${clampTurnForSummary(t.body)}`),
