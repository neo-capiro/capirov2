/**
 * Validation + safe merge for firm/user-authored Clio skills (P3-4).
 *
...
export function validateFirmSkill(
  ...
  let template: ClioSkill['template'] = null;
...
export function mergeSkills(
  ...
  firm: readonly ClioSkill[],
): ClioSkill[] {
  const claimed = new Set(builtIn.flatMap((s) => s.triggers));
  const out: ClioSkill[] = [...builtIn];
