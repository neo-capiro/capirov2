...
describe('buildClioSystemBlocks', () => {
  it('puts a single cache breakpoint on the static base block when caching is enabled', () => {
    const blocks = buildClioSystemBlocks({
      base: 'BASE PROMPT',
      dynamic: 'ctx',
      cacheEnabled: true,
    });
  ...
  it('omits all cache_control when caching is disabled', () => {
    const blocks = buildClioSystemBlocks({ base: 'BASE', dynamic: 'ctx', cacheEnabled: false });
  ...
  it('emits only the base block when the dynamic tail is empty or whitespace', () => {
    expect(buildClioSystemBlocks({ base: 'BASE', dynamic: '', cacheEnabled: true })).toHaveLength(
      1,
    );
    expect(
      buildClioSystemBlocks({ base: 'BASE', dynamic: '   \n ', cacheEnabled: true }),
    ).toHaveLength(1);
    expect(buildClioSystemBlocks({ base: 'BASE', cacheEnabled: true })).toHaveLength(1);
  });

  it('preserves base text byte-for-byte so the cached prefix is stable across turns', () => {
    const base = 'You are Clio.\n- rule one\n- rule two';
    const a = buildClioSystemBlocks({ base, dynamic: 'turn-1 context', cacheEnabled: true });
    const b = buildClioSystemBlocks({
      base,
  ...
  it('does not attach a breakpoint to an empty base', () => {
    const blocks = buildClioSystemBlocks({ base: '', dynamic: 'ctx', cacheEnabled: true });
