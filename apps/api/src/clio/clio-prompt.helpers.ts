...
export const CLIO_CAPABILITY_LINES: readonly string[] = [
  ...
  "- Your firm's own work: you can read workflows/submissions in flight, engagement tasks and what is overdue, strategies with their targets and deadlines, the Needs-Attention action items, tracked bills, regulatory dockets and comment deadlines, SAM.gov contract opportunities, meeting debriefs, outreach campaigns, and full client profiles (capabilities, key people, facilities, submission history). With the user's approval you can also create or update tasks and write approved values into workflow fields.",
  "- Populating client profiles from the web: when asked to research/import/fill in a client, you can research the organization (search_public_web, scrape_web_page, and federal sources like LDA filings) and then, AFTER showing the user the proposed values with a source for each and getting their approval, write them into the client profile via update_client_profile (name, website, description, productDescription, primary contact, sectorTag, issueCodes, uei, cageCode, naicsCodes, pscCodes). Never write unreviewed scraped data — propose first, write only what the user approves.",
