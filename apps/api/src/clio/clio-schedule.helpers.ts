/**
 * Pure, dependency-free helpers for Clio self-scheduled tasks (W3).
 *
 * No I/O / Prisma here so it unit-tests under `src/**.spec.ts`. The tool handler
 * (clio-tools.service.ts) and the runner (scripts/run-clio-scheduled-tasks.ts)
 * consume these.
 *
 ...
 * with the read-only tools in clio-tools.service.ts; deliberately EXCLUDES every
 ...
 * Mirrors SIDE_EFFECTING_TOOLS in clio-tools.service.ts.
 */
