/**
 * Pure helpers for the Meri execution trace (P1-3, observability).
 *
...
export interface MeriTraceUsage {
  inputTokens: number;
  outputTokens: number;
...
export interface MeriToolTrace {
  name: string;
  ok: boolean;
}

export interface MeriRoundTrace {
  round: number;
  durationMs: number;
  usage: MeriTraceUsage;
  stopReason: string | null;
  tools: MeriToolTrace[];
}

export interface MeriTurnTrace {
  intent: string;
  skill: string | null;
  roundCount: number;
...
export function summarizeTurnTrace(input: SummarizeTurnTraceInput): MeriTurnTrace {
  let toolCallCount = 0;
...
export function traceLogLine(t: MeriTurnTrace): string {
  const parts = [
    `intent=${t.intent}`,
    `skill=${t.skill ?? '-'}`,
    `rounds=${t.roundCount}`,
