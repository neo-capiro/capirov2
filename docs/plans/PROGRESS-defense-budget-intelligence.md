...
| 1.1 | P-1 procurement ingestion | ⏳ extract_pdoc.py ADAPTED — parses Army P-1 books → 35 line items w/ qty+$ (FY2024 $49,969K spot-verified vs PDF). BUT the P-Doc loader validates ALL codes via the RDT&E isValidPeCode (`^[0-9]{7}[A-Z]`), which rejects procurement BLINs (e.g. 9670A00005) → 100% quarantine. Pre-existing model gap (never exercised). LOADING PARKED (task spawned): procurement ≠ RDT&E PE — needs a deliberate model decision (own validation, or a separate procurement table). RDT&E pipeline unaffected |
